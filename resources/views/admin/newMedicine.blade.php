            ...
            <div class="col-lg-12">
              <div class="card">
                <div class="card-header d-flex align-items-center">
                  <h3 class="h4 mr-auto">Add New Medicine</h3>
                  <a class="btn btn-sm btn-primary" href="{{url('/')}}/admin/medicine/view">View All</a>
                </div>
                <div class="card-body">
                  <form class="form-horizontal" method="post" action="{{url('/')}}/admin/medicine/store" enctype="multipart/form-data">
                      {{ csrf_field() }}
                    <div class="form-group row">
                      ...
                      <div class="col-sm-6">
                        ...
                        <input type="text" name="description" class="form-control" value="{{old('desc')}}">
                      </div>
                    </div>
                    <div class="line"></div>
                    <div class="form-group row">
                      ...
                      <div class="col-sm-3">
                          ...
                          <input type="text" name="lorder" class="form-control" value="{{old('lorder')}}">
                      </div>
                    </div>
                    <div class="line"></div>
                    <div class="form-group row">
                                    ...
                                    <optgroup label="{{$cat->name}}">
                                      @if ($cat->subCategories->count())
                                          @foreach ($cat->subCategories as $scat)
                                              <option value="{{$scat->id}}" {{old('scat') == $scat->id ? "selected":""}}>{{$scat->name}}</option>
                                          @endforeach
                                      @endif
                                    </optgroup>
                                @endforeach
                          ...
                          <div class="col-sm-3 select">
                            <select name="unit" class="form-control">
                              ...
                              @foreach ($units as $unt)
                                  <option value="{{$unt->id}}"  {{old('unit') == $unt->id ? "selected":""}}>{{$unt->name}}</option>
                              @endforeach
                            </select>
                          </div>
                          <div class="col-sm-3 select">
                            <select name="generic" class="form-control">
                              ...
                              @foreach ($generics as $gen)
                                  <option value="{{$gen->id}}" {{old('generic') == $gen->id ? "selected":""}}>{{$gen->name}}</option>
                              @endforeach
                            </select>
                          </div>
                          <div class="col-sm-3 select">
                            <select name="power" class="form-control">
                              ...
                              @foreach ($powers as $pwr)
                                  <option value="{{$pwr->id}}" {{old('power') == $pwr->id ? "selected":""}}>{{$pwr->name}}</option>
                              @endforeach
                            </select>
                        </div>
                        </div>
